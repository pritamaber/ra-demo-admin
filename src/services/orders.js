 ...
 * Price:       fixed on the order date (see pricing.js). Later gold-rate changes never touch a placed order.
...
// ---------------------------------------------------------------- payments
/** Payments are plain rupees. Any number of them, but never more than what is still due. */
function addPayment(orderId, input) {
  return tx(() => {
    ...
    if (amount > order.outstanding_amount + EPS) {
      throw conflict(`${inr(amount)} is more than the ${inr(order.outstanding_amount)} due on this order.`, { outstanding: order.outstanding_amount });
    }

    q.run(
      `INSERT INTO payments (order_id, amount, payment_method, payment_date, reference_number, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      orderId, amount, input.payment_method, date, str(input.reference_number), str(input.notes), clock.timestamp());
    const paid = round2(order.paid_amount + amount);
    const outstanding = Math.max(round2(order.total_amount - paid), 0);
    q.run('UPDATE orders SET paid_amount = ?, outstanding_amount = ?, updated_at = ? WHERE id = ?', paid, outstanding, clock.timestamp(), orderId);
    logEvent(orderId, 'PAYMENT', `Payment of ${inr(amount)} received via ${input.payment_method}${input.reference_number ? ` (ref ${str(input.reference_number)})` : ''} — ${outstanding > EPS ? `${inr(outstanding)} still due` : 'paid in full'}`);
    return getOrder(orderId);
  });
}
 ...
 * same step. Delivery reduces stock and generates the final bill automatically.
 */
function deliver(id, input = {}) {
  return tx(() => {
    ...
    if (order.outstanding_amount > EPS) {
      throw conflict(`${inr(order.outstanding_amount)} is still due. Collect the full payment before handing over the piece.`, { outstanding: order.outstanding_amount });
    }

    const today = clock.today();
    q.run("UPDATE orders SET status = 'DELIVERED', actual_delivery_date = ?, updated_at = ? WHERE id = ?", today, clock.timestamp(), id);
    for (const it of itemsOf(id)) products.sell(it.product_id, it.quantity, id, `Delivered on ${order.order_number}`);
    logEvent(id, 'DELIVERED', `Delivered to the customer — ${inr(order.total_amount)} paid in full`);
...
function priceBreakdown(order, items) {
  return {
    ...
    lines: items.map((it) => ({
      ...
      gold_rate: it.gold_rate, gold_value: it.gold_value, making_rate: it.making_rate, making_charge: it.making_charge, gst: it.gst, total: it.total,
    })),
  };
}
...
function getOrder(id) {
  ...
  return {
    ...
    payments: payments.map((p, i) => ({ ...p, kind: kinds[i] })),
...
module.exports = {
  ...
  previewOrder, createOrder, acceptOrder, setReady, updateDeliveryDate, addPayment, deliver, cancelOrder,
  getOrder, listOrders, orderRow, itemsOf, paymentsOf, logEvent, paymentKinds,
};
